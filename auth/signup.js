...
document.addEventListener('DOMContentLoaded', function() {
    ...
    signupForm.addEventListener('submit', function(e) {
        ...
        if (!userRole) {
            showError('Please select whether you are registering as a fan or artist');
            return;
        }
        ...
        .then(({ ok, status, body }) => {
            ...
            // Also set as current user in runtime session helper (server session will be authoritative after login)
            if (window.afro && typeof window.afro.setUser === 'function') {
                window.afro.setUser({
                    email: email,
                    name: fullName,
                    isLoggedIn: true,
